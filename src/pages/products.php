...
<head>
  ...
  <title>Products - Promise Shop</title>
...
<!-- HERO -->
<div class="bg-gradient-to-br from-pink-50 to-rose-50 py-20">
    <div class="container mx-auto px-4 text-center">
        <h1 class="font-Tinos text-5xl md:text-6xl text-pink-950 mb-6">
            Our <span class="text-candy-peach">Products</span>
        </h1>
        <p class="font-unna text-xl text-pink-700 max-w-3xl mx-auto leading-relaxed">
            Browse our curated collection of wedding attire crafted for elegance, comfort and timeless memories.
        </p>
        <p class="font-unna text-lg text-pink-600 mt-4"><?=$total_products?> Products Available</p>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <?php else: ?>
            <?php foreach ($products as $product): ?>
              <article class="product-card flex flex-col overflow-hidden rounded-lg border border-pink-200 bg-white text-pink-950 shadow-[0_8px_30px_rgba(236,72,153,0.08)]">
                <div class="relative h-56 overflow-hidden sm:h-64 md:h-72 lg:h-80">
                  <a href="product-detail.php?id=<?=$product['product_id']?>">
                    <img src="../<?=str_replace('src/img/', 'img/', $product['image_path'])?>" alt="<?=htmlspecialchars($product['product_name'])?>" class="h-full w-full object-cover hover:scale-105 transition-transform duration-300">
                  </a>
                  <button class="absolute top-3 right-3 rounded-full bg-white/80 p-2 text-pink-600 shadow hover:bg-white transition-colors">
                    ♡
                  </button>
                </div>
                  ...
                  <div class="mt-4 flex items-center justify-between">
                    ...
                    <?php if ($isLoggedIn): ?>
                      <form action="cart.php" method="post" class="inline">
                        ...
                        <button type="submit" class="cursor-pointer rounded-md bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2 text-white transition hover:shadow-[0_0_30px_rgba(236,72,153,0.25)] hover:scale-105">
                          Add to Cart
                        </button>
                      </form>
                    <?php else: ?>
                      <button type="button" onclick="openModal('login-modal')" class="cursor-pointer rounded-md bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2 text-white transition hover:shadow-[0_0_30px_rgba(236,72,153,0.25)] hover:scale-105">
                        Add to Cart
                      </button>
                    <?php endif; ?>
                  </div>
        ...
        <?php if ($total_products > $num_products_on_each_page): ?>
          <div class="flex justify-center items-center mt-12 space-x-4">
            <?php if ($current_page > 1): ?>
              <a href="products.php?p=<?=$current_page-1?>" class="px-4 py-2 bg-pink-200 text-pink-800 rounded-lg hover:bg-pink-300 transition-colors">
                Previous
              </a>
            <?php endif; ?>
