                ...
                <?php foreach ($orders as $order): ?>
                    <div class="card">
                        <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-dark">Order #<?= (int) $order['order_id']; ?></h3>
                                <p class="text-sm text-neutral">Placed on <?= date('F d, Y', strtotime($order['order_date'])); ?></p>
                            </div>
                            <span class="self-start rounded-full px-3 py-1 text-sm font-medium <?= resolveStatusBadge($order['status']); ?>">
                                ...
                                <?php foreach ($orderItems[$order['order_id']] as $item): ?>
                                    <div class="flex items-center gap-3 rounded-xl border border-light bg-gray-50 p-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-sm text-neutral">Quantity: <?= (int) $item['quantity']; ?> × ₱<?= number_format($item['unit_price'], 2); ?></p>
                                        </div>
                        ...
                        <div class="flex flex-col gap-4 border-t border-light pt-4 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row">
                                ...
                                <a href="../backend/order_receipt.php?id=<?= (int) $order['order_id']; ?>" class="btn-primary" target="_blank" rel="noopener">Download Receipt</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
